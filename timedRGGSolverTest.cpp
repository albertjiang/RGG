...
int main() {
  ...
  r->multiLinearSolve();
  auto t10 = Clock::now();
  auto solveRGGTime = std::chrono::duration_cast<std::chrono::nanoseconds>(t10 - t9).count();
  std::cout << "Time Taken to Find Equilibrium for The Resource Graph version of Game: " 
              << solveRGGTime
